...
def genQueue(name: str,base:int = 4,index:int = 8):
  """
  ユーザー定義キューを追加するプログラム
...
# データをheadに設定
data modify storage txqueue:{name} data[-1]{"[-1]"*index}._ set from storage txqueue: IO
...
# データをtailから取得
data modify storage txqueue: IO set from storage txqueue:{name} data[0]{"[0]"*index}._
